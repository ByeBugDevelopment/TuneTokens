...
var TuneToken = artifacts.require("../contracts/TuneToken.sol")

contract("TuneToken", (accounts) => {
    it("should have the name TUNE Token", () => {
        return TuneToken.deployed().then((instance) => {
            return instance.name.call();
        })
            .then((name) => {
                assert.equal(name, "TUNE Token", "TUNE Token wasn't the name")
            });
    });

    it("should have the symbol TUNE", () => {
        return TuneToken.deployed().then((instance) => {
            return instance.symbol.call()
        })
            .then((symbol) => {
                assert.equal(symbol, "TUNE", "TUNE wasn't the symbol")
            })
    })
